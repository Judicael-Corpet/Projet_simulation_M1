...
P_fixe2 = Particule(p0=V3D(monUnivers.dimensions[0]/2+10,monUnivers.dimensions[1]/2),fix=True)
P_osc = Particule(p0=V3D(monUnivers.dimensions[0]/2+1,monUnivers.dimensions[1]/2 - 10))
P_double = Particule(p0=V3D(monUnivers.dimensions[0]/2+11,monUnivers.dimensions[1]/2 - 10),color=(0,0,1))

grav = Gravity(V3D(0,-10))
boing = Bounce_y(.9,monUnivers.step) 
boing2 = Bounce_x(1,monUnivers.step) 

link = SpringDamper(P_fixe,P_osc,k=1000,l0=10, c=100)
link2 = SpringDamper(P_fixe2,P_double,k=1000,l0=10, c=100)
ressort = SpringDamper(P_double,P_osc,k=10,l0=10, c=.1)


forceX5 = ForceSelect(V3D(5,0),[P_osc,P_double])
forceXN5 = ForceSelect(V3D(-5,0),[P_osc,P_double])
forceY15 = ForceSelect(V3D(0,15),[P_osc,P_double])
forceYN15 = ForceSelect(V3D(0,-15),[P_osc,P_double]) 

monUnivers.addParticule(P_fixe,P_fixe2,P_osc,P_double)
monUnivers.addGenerators(grav,boing,boing2)
monUnivers.addGenerators(link,link2,ressort)
...
def myInteraction(self,events,keys):
	...
	# post-process
	monUnivers.out1.append( P_osc.getPosition().x - P_osc.position[0].x)
	monUnivers.out2.append( P_double.getPosition().x - P_double.position[0].x +2)
